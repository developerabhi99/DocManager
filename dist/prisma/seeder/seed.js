...
async function main() {
    ...
    const adminRole = await prisma.role.create({
        data: {
            ...
            permissions: {
                create: permissions.filter(p => ['MANAGE_USERS', 'MANAGE_APPOINTMENTS', 'VIEW_REPORTS', 'MANAGE_SCHEDULE'].includes(p.key)).map(p => ({
                    permission: { connect: { id: p.id } }
                }))
            }
        }
    ...
    const doctorRole = await prisma.role.create({
        data: {
            ...
            permissions: {
                create: permissions.filter(p => ['MANAGE_APPOINTMENTS', 'VIEW_REPORTS', 'MANAGE_SCHEDULE'].includes(p.key)).map(p => ({
                    permission: { connect: { id: p.id } }
                }))
            }
        }
    ...
    const staffRole = await prisma.role.create({
        data: {
            ...
            permissions: {
                create: permissions.filter(p => ['MANAGE_APPOINTMENTS', 'VIEW_REPORTS'].includes(p.key)).map(p => ({
                    permission: { connect: { id: p.id } }
                }))
            }
        }
