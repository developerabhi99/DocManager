...
export const getUsersCount = async (req: Request, res: Response) => {
  try {
    ...
    const usersByRole = await prisma.$queryRaw<Array<{roleName: string, count: string}>>`
      SELECT 
        r.name as "roleName",
        COUNT(u.id) as "count"
      FROM "User" u
...
export const getAppointmentsCount = async (req: Request, res: Response) => {
  try {
    ...
    const appointmentsByStatus = await prisma.$queryRaw<Array<{status: string, count: string}>>`
      SELECT 
        status,
        COUNT(id) as "count"
      FROM "Appointment"
...
export const getReportsCount = async (req: Request, res: Response) => {
  try {
    ...
    const reportsByMonth = await prisma.$queryRaw<Array<{month: Date, count: string}>>`
      SELECT 
