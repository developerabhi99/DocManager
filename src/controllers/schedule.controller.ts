...
export async function upsertDoctorSchedule(req: Request, res: Response) {
  const { doctorId } = req.params;
...
export async function getEmployeeAvailability(req: Request, res: Response) {
  ...
  try {
    ...
    const appointments = await prisma.appointment.findMany({
      where: {
        referredTo: employeeId as string,
        dateTime: {
          gte: startOfDay,
          lte: endOfDay
        },
...
export async function getAllDoctorSchedules(req: Request, res: Response) {
  const authUser: any = (req as any).user;
